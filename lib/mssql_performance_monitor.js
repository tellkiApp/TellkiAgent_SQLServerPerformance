...
metrics["LockWaits"] = {id:"9:Lock Waits/Sec:7",value:"",ratio:true,query:"SELECT cntr_value [LockWaits] FROM sys.dm_os_performance_counters " + "WHERE counter_name = 'Lock Waits/sec' " + "AND instance_name = '_Total' AND object_name LIKE '%Locks%'"};
...
function getDataValue(connection, connRequest, metric, metricName, request)
{
	connRequest.query(metric.query, function(err, recordset) {
		...
		{
			var e = new DatabaseConnectionError(err.message);
			e.message = "Unable to retrieve metrics value from query.";
			errorHandler(e);
		}
